...
module SlideRules.Renderer.FastSVG where
...
instance Renderer FastSVG where
    type Representation FastSVG = Element
    renderTick _ = tickToElement
    renderTickStatic _ = tickToElementStatic
    renderTicks proxya renderSettings =
        svg . foldMap (renderTick proxya renderSettings)
    writeRepToFile _ path rep = do
        writeFile path $ show rep
...
tickToElement :: RenderSettings -> Tick -> Element
...
tickToElementStatic :: RenderSettings -> Tick -> Element
tickToElementStatic RenderSettings{ heightMultiplier, textMultiplier } tick =
